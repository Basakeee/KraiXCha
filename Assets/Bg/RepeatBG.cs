...
public class RepeatBG : MonoBehaviour
{
    ...
    {
        float objectHeight = obj.GetComponent<SpriteRenderer>().bounds.size.y - choke; // ใช้ความสูงแทน
        int childsNeeded = (int)Mathf.Ceil(screenBounds.y * 2 / objectHeight); // คำนวณจำนวนลูกที่ต้องสร้าง
        ...
        for (int i = 0; i <= childsNeeded; i++)
        {
            ...
            c.transform.position = new Vector3(obj.transform.position.x, objectHeight * i, obj.transform.position.z); // เปลี่ยนตำแหน่งเป็นแกน Y
            c.name = obj.name + i;
        }
        Destroy(clone);
    ...
    void repositionChildObjects(GameObject obj)
    {
        ...
        if (children.Length > 1)
        {
            ...
            float halfObjectHeight = lastChild.GetComponent<SpriteRenderer>().bounds.extents.y - choke; // ใช้ความสูงแทน
    ...
    void Update()
    {
        ...
        Vector3 desiredPosition = transform.position + new Vector3(0, scrollSpeed, 0); // เลื่อนในแกน Y
